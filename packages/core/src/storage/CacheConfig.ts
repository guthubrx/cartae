...
export interface CacheConfig {
  ...
  // ========================================================================
  // Politique de Nettoyage (Pruning)
  // ========================================================================
 ...
 */
export const GENEROUS_CACHE_CONFIG: CacheConfig = {
  maxItems: 1000,
  maxSizeMB: 300,
  maxAgeDays: 60,
 ...
 */
export function estimateItemSizeMB(item: any): number {
 ...
 */
export function getItemType(item: any): ItemType {
  const type = item.type?.toLowerCase() || 'other';

  if (['email', 'message'].includes(type)) return 'email';
  if (['task', 'todo'].includes(type)) return 'task';
  if (['note', 'document'].includes(type)) return 'note';
  if (['event', 'calendar'].includes(type)) return 'event';
